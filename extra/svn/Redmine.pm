              ...
              WHERE 
                ...
                AND (  
                  (
                    ...
                    OR (roles.builtin=1 AND cast(projects.is_public as CHAR) IN ('t', '1'))
                  )
                  OR (
                    roles.id IN (SELECT global_roles.role_id FROM global_roles WHERE global_roles.role_id = roles.id AND global_roles.user_id = users.id)
                    AND cast(projects.is_public as CHAR) IN ('t', '1')
                  )
                )
